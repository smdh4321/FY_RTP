...
instruction_label = ttk.Label(app, text="ENTER THE LOOKUP TYPE TO FETCH THE DATA" , foreground="black", font=font.Font(size=11), background="#CCCCCC")
...
def fetch_lookup_data(event=None):
    lookup_type = entry_fields[0].get()  # Assuming the LOOKUP_TYPE entry field is the first one
    if lookup_type:  # Check if lookup_type is not empty
        try:
            ...
            cursor.execute("SELECT TYPE_DESCRIPTION, ENABLED_FLAG FROM Lookup_Type WHERE LOOKUP_TYPE = ?", (lookup_type,))
            row = cursor.fetchone()
            ...
            if row and not entry_fields[1].get():
                type_description, enabled_flag = row
                entry_fields[1].insert(0, type_description)  # Populate TYPE_DESCRIPTION
                entry_fields[2].set(enabled_flag)  # Set the value of ENABLED_FLAG ComboBox
...
def Modify_lookup_type():
    try:
        ...
        lookup_type = entry_fields[0].get()

        # Check if the lookup type already exists in the database
        cursor = connection.cursor()
        cursor.execute("SELECT COUNT(*) FROM Lookup_Type WHERE LOOKUP_TYPE = ?", (lookup_type,))
        ...
        if count > 0:
            ...
            query = """
                UPDATE Lookup_Type 
                SET TYPE_DESCRIPTION = ?, ENABLED_FLAG = ?, LAST_UPDATE_DATE = ?, LAST_UPDATED_BY_USER= ?
                WHERE LOOKUP_TYPE = ?
            """
            query_params = (entry_fields[1].get(), entry_fields[2].get(), current_date, username, lookup_type)
        else:
            # If lookup type does not exist, perform insert
            query = """
                INSERT INTO Lookup_Type 
                (LOOKUP_TYPE, TYPE_DESCRIPTION, ENABLED_FLAG, CREATION_DATE, LAST_UPDATE_DATE) 
                VALUES (?, ?, ?, ?, ?)
            """
            query_params = (lookup_type, entry_fields[1].get(), entry_fields[2].get(), current_date, current_date)
