...
def fetch_lookup_data(event=None):
    lookup_type_id = entry_fields[0].get()  # Assuming the LOOKUP_TYPE_ID entry field is the first one
    if lookup_type_id:  # Check if lookup_type_id is not empty
        try:
            ...
            cursor.execute("SELECT LOOKUP_CODE, LOOKUP_VALUE, VALUE_DESCRIPTION, ENABLED_FLAG FROM Lookup_Values WHERE LOOKUP_TYPE_ID = ?", (lookup_type_id,))
            row = cursor.fetchone()

            # If data is found and LOOKUP_VALUE has not been fetched yet, populate the entry fields
            if row and not entry_fields[2].get():
                lookup_code, lookup_value, value_description, enabled_flag = row
                entry_fields[1].insert(0, lookup_code)  # Populate LOOKUP_CODE
                entry_fields[2].insert(0, lookup_value)  # Populate LOOKUP_VALUE
                entry_fields[3].insert(0, value_description)  # Populate VALUE_DESCRIPTION
                entry_fields[4].set(enabled_flag)  # Set the value of ENABLED_FLAG ComboBox
...
def Modify_lookup_values():
    try:
        ...
        cursor = connection.cursor()
        cursor.execute("SELECT COUNT(*) FROM Lookup_Values WHERE LOOKUP_TYPE_ID = ?", (lookup_type_id,))
        ...
        if count > 0:
            ...
            query = """
                ...
                SET LOOKUP_CODE = ?, LOOKUP_VALUE = ?, VALUE_DESCRIPTION = ?, ENABLED_FLAG = ?, LAST_UPDATE_DATE = ?, LAST_UPDATED_BY_USER= ?
                WHERE LOOKUP_TYPE_ID = ?
            """
            query_params = (entry_fields[1].get(), entry_fields[2].get(), entry_fields[3].get(), entry_fields[4].get(), current_date, username, lookup_type_id)
        else:
            # If lookup type does not exist, perform insert
            query = """
                INSERT INTO Lookup_Values
                (LOOKUP_TYPE_ID, LOOKUP_CODE, LOOKUP_VALUE, VALUE_DESCRIPTION, ENABLED_FLAG, CREATION_DATE, LAST_UPDATE_DATE) 
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """
            query_params = (lookup_type_id, entry_fields[1].get(), entry_fields[2].get(), entry_fields[3].get(), entry_fields[4].get(), current_date, current_date)
...
info_label_inventory = ttk.Label(app, text="3S Technologies - LOOKUP ")
